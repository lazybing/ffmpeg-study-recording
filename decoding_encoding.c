...
static void audio_encode_example(const char *filename)
{
    ...
    if(!check_sample_fmt(codec, c->sample_fmt)){
        fprintf(stderr, "Encoder does not support sample formt %s",
    ...
    c->chanel_layout = select_channel_layout(codec);
...
static void audio_decode_example(const char *outfilename, const char *filename)
{
    ...
    int len;
    FILE *f, *outfile;
    uin8_t inbuf[AUDIO_INBUF_SIZE + AV_INPUT_BUFFER_PADDING_SIZE];
    AVPacket avpkt;
    ...
    while(avpkt.size > 0){
        ...
        if(got_frame){
            ...
            for(i = 0; i < decode_frame->nb_channels; i++)
                for(ch = 0; ch < c->channels; ch++)
                    fwrite(decode_frame->data[ch] + data_size*i, 1, data_size, outfile);
        }
        ...
        if(avpkt.size < AUDIO_REFILL_THRESH){
            /*
             * Refill the input buffer, to avido trying to decode
             * incomplete frames.Instead of this, one could also use
             * a parser, or use a proper container format through libavformat
             */
...
static void video_encode_example(const char *filename, int codec_id)
{
    ...
    //emit one intra frmae every ten frames
    //check frame pic_type before passing frame
    //to encoder, if frame->pict_type is AV_PICTURE_TYPE_I
    //then top_size is ignored and the output of encoder 
    ...
    for(i = 0; i < 25; i++){
        av_init_packet(&pkt);
        pkt.data = NULL; //package data will be allocated by the encoder
        pkt.size = 0;
